...
// GlpManagerTestIterator is returned from FilterTest and is used to iterate over the raw logs and unpacked data for Test events raised by the GlpManager contract.
type GlpManagerTestIterator struct {
	Event *GlpManagerTest // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *GlpManagerTestIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *GlpManagerTestIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *GlpManagerTestIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// GlpManagerTest represents a Test event raised by the GlpManager contract.
type GlpManagerTest struct {
	Arg *big.Int
	Raw types.Log // Blockchain specific contextual infos
}
...
//
// Solidity: event Test(uint256 arg)
func (_GlpManager *GlpManagerFilterer) FilterTest(opts *bind.FilterOpts) (*GlpManagerTestIterator, error) {

	logs, sub, err := _GlpManager.contract.FilterLogs(opts, "Test")
	if err != nil {
		return nil, err
	}
	return &GlpManagerTestIterator{contract: _GlpManager.contract, event: "Test", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event Test(uint256 arg)
func (_GlpManager *GlpManagerFilterer) WatchTest(opts *bind.WatchOpts, sink chan<- *GlpManagerTest) (event.Subscription, error) {

	logs, sub, err := _GlpManager.contract.WatchLogs(opts, "Test")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(GlpManagerTest)
				if err := _GlpManager.contract.UnpackLog(event, "Test", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Test(uint256 arg)
func (_GlpManager *GlpManagerFilterer) ParseTest(log types.Log) (*GlpManagerTest, error) {
	event := new(GlpManagerTest)
	if err := _GlpManager.contract.UnpackLog(event, "Test", log); err != nil {
		return nil, err
	}
